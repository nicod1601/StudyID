...
- Les comptes sont stockés **en clair** dans `comptes.data` (JSON) — pense à
  ...
  un, sinon ajoute-le).
- Le chat est un salon commun (pas de messages privés) : tout le monde de
  connecté voit tous les messages et fichiers échangés.
- Les fichiers (y compris `.zip`) sont relayés en direct, sans être stockés
  sur le serveur : seules les personnes connectées **au moment de l'envoi**
  les reçoivent.
